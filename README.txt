...
1) Create an HTML mock-up of the pricing UI. The interface needs to fulfill the following requirements:

It must display all the products in the inquiry.
It must allow the user to select from three pricing "levels": Deal, Title, and Product 
It must allow the user to input price(s) based upon the selected pricing level:
Deal level: there is a single flat price for the entire collection of products.
Title level: each group of Titles is priced separately. For example, the three Amile products above would be given a single grouped price.
Product level: each product is priced individually.
		
Your mock-up should be styled with CSS, but may use the static data provided above (you can hard-code display values,	but the interface should respond appropriately to any user interaction on the screen).
	
2) Dynamically implement your UI client-side. The interface should work for any collection of products sent to you from the server.    If you wish, you may use any JavaScript-based front-end libraries or frameworks you are familiar with (AngularJs, JQuery, underscore, etc.).
